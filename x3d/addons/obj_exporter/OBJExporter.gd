...
var progress_update_frequency: int = 1000
...
func save_mesh_to_files(mesh: Mesh, file_path: String, object_name: String) -> void:
	...
	export_started.emit.call_deferred()
	...
	for s in range(mesh.get_surface_count()):
		...
		while i < indices_count:
			...
			if (i % progress_update_frequency * 3) == 0: # Modulo must be multiple of 3 as it's the step
				export_progress_updated.emit.call_deferred(s, i / float(indices_count))
				await get_tree().process_frame
			i += 3

		export_progress_updated.emit.call_deferred(s, 1.0)
	...
	export_completed.emit.call_deferred(obj_file, mat_file)
