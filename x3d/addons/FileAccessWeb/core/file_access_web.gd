...
func open(accept_files: String = "*", key_in_record: String = "") -> void:
	if _is_not_web():
		_notify_error()
		return
	
	_file_uploading.setKey(key_in_record)
...
func _on_file_loaded(args: Array) -> void:
	var file_name: String = args[0]
	var bytes: String = str(args[1])
	loaded.emit(file_name, "", bytes)
	...
	input.onchange = (event) => {
		...
		reader.onload = (readerEvent) => {
			if (readerEvent.target.readyState === FileReader.DONE) {
				if (recordKey != "") {
					loadedCallback(file.name, readerEvent.loaded);
					fileContents[recordKey] = readerEvent.target.result
				}
			}
		}
